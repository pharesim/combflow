...
| `prerender` | Headless Chromium prerender for bot/crawler SSR — Caddy routes bot user-agents here for fully-rendered HTML (1G memory limit). If prerender stalls or returns a 5xx, Caddy falls back to the app's server-rendered page so crawlers get substantive HTML instead of a soft-404 |
...
- **Privacy-preserving image proxy** — all user-facing images (avatars, thumbnails, post body images) are routed through a server-side proxy (`/api/imageproxy`) so external image hosts never see user IPs or browser fingerprints. The proxy only serves raster image types (SVG is refused, since a directly-opened SVG can run scripts) and refuses to fetch from non-public addresses (SSRF protection). Cloudflare edge-caches proxied images for performance. CSP restricts `img-src` to the app origin, `data:` URIs, and OpenStreetMap map tiles.
...
- **Social previews** — post-specific Open Graph meta tags on deep links (title, description, thumbnail fetched from Hive API) for rich previews on Discord, Twitter, Slack, etc. Author profile pages with 10+ classified posts get a dynamic per-author title and description (e.g. `@user — 247 posts on Hive · photography, travel`).
- **Server-rendered page enrichments** — for crawlers and no-JS visitors, every indexable surface injects unique-per-page HTML server-side (the SPA replaces it once JavaScript loads). The home, category (`/c/`), language (`/lang/`), and community (`/community/`) pages render a recent-posts list (titles, authors, dates, excerpts) plus a topic/language/community intro paragraph and `ItemList` structured data — visible above the grid on filtered surfaces, hidden on the homepage. Post pages render the post body as plain text, an "About the author" mini-card, and the top comments inline, with `Comment` structured data. Author profiles (10+ posts) lead with a summary block — total posts, "active since", top categories/languages/community — plus a recent-posts list (the crawler/no-JS floor, replaced by the live grid once JavaScript loads) and `Person` structured data. This server-rendered floor is also what Caddy serves bots when prerender fails. Data is fetched concurrently so the extra content adds no render latency.
...
│   │               └── preferences.js, communities.js, editor.js, location.js, notifications.js, mentions.js
│   ├── db/

...
- **Server-rendered page enrichments** — for crawlers and no-JS visitors, post and author pages inject unique-per-page HTML server-side (the SPA modal replaces it once JavaScript loads). Author profiles (10+ posts) lead with a summary block — total posts, "active since", top categories/languages/community — plus `Person` structured data. Post pages render an "About the author" mini-card and the top comments inline (plain-text excerpts) with `Comment` structured data. Author and comment data are fetched concurrently with the post so the extra content adds no render latency.
